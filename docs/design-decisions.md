...
| D31 | 新手引导演示沙箱：fixtures 只读 + tour 状态隔离 + 认知横幅（M12） | ①地基事实（本次侦察实证）：后端**不存在任何 /api/fixtures 路由**（app.py 全文与 14 个 router 均无），前端假数据为视图内嵌 LOCAL_FIXTURE_*（web/tests/jobs.test.mjs:18 证），交互全客户端 mock——写请求打到不存在路由即 404，物理上不可能触达 SQLite/画像文件；python 侧 api/fixtures.py 仅 pytest 消费；②三红线：**fixtures 永不获得后端路由**（防御测试钉死为契约，杜绝未来"顺手加个 demo 写端点"）；**tour 状态只用 tf_tour_* 命名空间**（进入时保存并退出时还原 tf_real——唯一可能被污染的真实开关）；**演示期全程醒目横幅**（"演示数据——不会写入你的真实数据"，认知污染与数据污染同权重）；③复用而非新建：引导素材直接用 LOCAL_FIXTURE_*（决策卡/主张卡/三态岗位/八格画像各配一步讲解），不造第二套演示数据 |
