...
class QrLoginSession:
    ...
    async def _refresh_qr(self) -> None:
        """截图二维码元素为 PNG base64（元素未出现时整页截图兜底）。"""
        if self._page is None:
            return
        try:
            qr = self._page.locator(QR_SELECTOR).first
            await qr.wait_for(state="visible", timeout=10_000)
            png = await qr.screenshot()
        except Exception:  # noqa: BLE001 — 二维码选择器变化时整页兜底
    ...
    async def _poll_loop(self) -> None:
        ...
        while asyncio.get_event_loop().time() < deadline:
            ...
            if any(c["name"] in LOGIN_COOKIE_KEYS for c in cookies):
                ...
                await self.close()
                return
            # 二维码约 2 分钟过期：定期重截（Boss 过期后元素 src 会换）
